...
- **Validation rules**: `.min(n)`, `.max(n)`, `.nonempty()`, `.email()`, `.url()`, `.regex(pattern)`, `.startsWith(prefix)`, `.endsWith(suffix)`, `.includes(substring)`, `.uuid()`, `.int()`, `.positive()`, `.negative()`, `.multipleOf(n)` — all with optional custom error message `msg?` parameter
...
- **LLM prompts**: `schema_to_prompt()` auto-generates TypeScript-interface prompt text with constraint comments
...
| `intersection(Array[Schema])` | Intersection — passes if all schemas match; object fields are merged |
...
| `.email([msg])` | string | Validates email format (improved: single @, no leading/trailing dots, domain has dot) |
| `.url([msg])` | string | Must start with `http://` or `https://` |
...
```bash
moon test                # Run all tests (206 total, 0 warnings)
